import { WORD_LIST, getWordForDate } from '../wordList';
...
function sharedLetters(word1: string, word2: string): number {
...
function analyzeWordSimilarity(word1: string, word2: string) {
...
function getWordsForDateRange(startDate: Date, days: number) {
    const words = [];
    for (let i = 0; i < days; i++) {
        ...
        words.push({
            ...
            word,
            index: Math.floor((date.getTime() - new Date('2024-01-01').getTime()) / (1000 * 60 * 60 * 24)) % WORD_LIST.length
        });
    }
    return words;
}
...
Object.keys(lengthDistribution).sort((a, b) => parseInt(a) - parseInt(b)).forEach(length => {
    ...
    const percentage = ((count / WORD_LIST.length) * 100).toFixed(1);
...
const clusters: Array<{ start: number, words: string[], avgSimilarity: number }> = [];

for (let i = 0; i < WORD_LIST.length - 2; i++) {
    const word1 = WORD_LIST[i];
    const word2 = WORD_LIST[i + 1];
    const word3 = WORD_LIST[i + 2];
...
const pondIndex = WORD_LIST.indexOf('POND');
const ponyIndex = WORD_LIST.indexOf('PONY');
const poolIndex = WORD_LIST.indexOf('POOL');
