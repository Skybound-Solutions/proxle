...
function createBar(value, max = 1, width = 40) {
...
for (let i = 0; i < days; i++) {
    ...
    let similarity = 0;
    if (prevWord) {
        similarity = (0, wordList_1.calculateWordSimilarity)(prevWord, word);
    ...
    console.log(`${dateStr} | ${word.padEnd(9)} | ${word.length}      | ${similarityBar} ${similarityText}${warning}`);
    prevWord = word;
}
// Statistics
console.log('\n📊 STATISTICS:\n');
...
for (let i = 0; i < words.length - 2; i++) {
    const sim1 = (0, wordList_1.calculateWordSimilarity)(words[i].word, words[i + 1].word);
    const sim2 = (0, wordList_1.calculateWordSimilarity)(words[i + 1].word, words[i + 2].word);
    if (sim1 > 0.6 && sim2 > 0.6) {
        issues.push(`${words[i].date} - ${words[i + 2].date}: ` +
...
console.log('\n✅ Analysis complete!');
//# sourceMappingURL=visualizeWordVariety.js.map