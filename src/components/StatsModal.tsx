...
import { X, Share2, Shield, Check, Edit2 } from 'lucide-react';
...
export default function StatsModal({
    ...
    const [formData, setFormData] = useState({
        leaderboardName: userData?.leaderboardName || '',
        message: userData?.message || '',
        displayOnLeaderboard: userData?.displayOnLeaderboard !== false,
        showAmount: userData?.showAmount || 'exact'
    });
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
                ...
                {/* Supporter Section */}
                {isSupporter && (
                    <div className="mb-8 p-4 bg-cyan-500/10 rounded-xl border border-cyan-500/20">
                        ...
                        {isEditing ? (
                            <div className="space-y-4">
                                <div>
                                    <label className="text-[10px] uppercase text-white/40 block mb-1">Leaderboard Name</label>
                                    <input
                                        type="text"
                                ...
                                <div>
                                    <label className="text-[10px] uppercase text-white/40 block mb-1">Billboard Message (Top 3)</label>
                                    <textarea
                                        ...
                                        placeholder="Add a custom shoutout..."
                                    />
                                </div>
                                <div className="flex items-center justify-between">
                                    <label className="text-xs text-white/70">Show on Leaderboard</label>
                                    <button
                                ...
                                <button
                                    ...
                                    className="w-full py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-all"
                                >
                                    {saveStatus === 'saving' ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> :
                                        saveStatus === 'saved' ? <Check size={18} /> : 'Save Profile'}
                                </button>
                            </div>
                        ) : (
                            <div className="space-y-2">
                                ...
                                {formData.message && (
                                    <div className="text-xs italic text-cyan-400 opacity-80">"{formData.message}"</div>
                                )}
